...
/**
 * <a
 * href="https://github.com/ben-manes/caffeine/blob/master/caffeine/src/jmh/java/com/github/benmanes/caffeine/cache/MemoryBenchmark.java#L41">...</a>
 *
 * <p>A non-JMH benchmark to compare the memory overhead of different cache implementations. Note
 * that the measurements estimate based on the current JVM configuration, e.g. 64-bit with
 * compressed references if the benchmark is executed with a heap under 32GB. This can mean that
 * object padding may or may not have a visible effect.
 *
 * <p>This benchmark requires a JavaAgent to evaluate the object sizes and can be executed using
...
public final class MemoryBenchmark {
  ...
  static final ImmutableMap<Integer, Integer> workingSet =
      IntStream.range(0, FUZZY_SIZE).boxed().collect(toImmutableMap(identity(), i -> -i));
  ...
  private void maximumSize() {
    ...
    com.google.common.cache.Cache<Integer, Integer> guava =
        CacheBuilder.newBuilder().maximumSize(MAXIMUM_SIZE).build();
    compare("Maximum Size", caffeine, guava);
  }

  private void maximumWeight() {
    Cache<Integer, Integer> caffeine =
        builder().maximumWeight(MAXIMUM_SIZE).weigher((k, v) -> 1).build();
    com.google.common.cache.Cache<Integer, Integer> guava =
        CacheBuilder.newBuilder().maximumWeight(MAXIMUM_SIZE).weigher((k, v) -> 1).build();
    compare("Maximum Weight", caffeine, guava);
  }

  private void maximumSize_expireAfterAccess() {
    Cache<Integer, Integer> caffeine =
        builder().expireAfterAccess(Duration.ofMinutes(1)).maximumSize(MAXIMUM_SIZE).build();
    com.google.common.cache.Cache<Integer, Integer> guava =
  ...
  private void maximumSize_expireAfterWrite() {
    Cache<Integer, Integer> caffeine =
        builder().expireAfterWrite(Duration.ofMinutes(1)).maximumSize(MAXIMUM_SIZE).build();
    com.google.common.cache.Cache<Integer, Integer> guava =
        CacheBuilder.newBuilder()
  ...
  private void maximumSize_refreshAfterWrite() {
    Cache<Integer, Integer> caffeine =
        builder().refreshAfterWrite(Duration.ofMinutes(1)).maximumSize(MAXIMUM_SIZE).build(k -> k);
    com.google.common.cache.Cache<Integer, Integer> guava =
        CacheBuilder.newBuilder()
  ...
  private void expireAfterAccess() {
    Cache<Integer, Integer> caffeine = builder().expireAfterAccess(Duration.ofMinutes(1)).build();
    com.google.common.cache.Cache<Integer, Integer> guava =
  ...
  private void expireAfterWrite() {
    Cache<Integer, Integer> caffeine = builder().expireAfterWrite(Duration.ofMinutes(1)).build();
    com.google.common.cache.Cache<Integer, Integer> guava =
  ...
  private void compare(
    ...
    String result =
        FlipTable.of(
            new String[] {"Cache", "Baseline", "Per Entry"},
            new String[][] {
  ...
  private String[] evaluate(String label, Map<Integer, Integer> map) {
    ...
    long perEntry =
        LongMath.divide(populated - entryOverhead - base, FUZZY_SIZE, RoundingMode.HALF_EVEN);
