...
/**
 * Record the total size in bytes of active objects on the heap after the last benchmark iteration
 * and print a heap histogram. The measure is determined by calling {@code jmap} with the PID of the
 * benchmark process. Since calling jmap will result in a garbage collection in the VM running the
 * benchmark, this is never done in between iterations.
 *
 * @author Jens Wilke
 */
@Slf4j
  ...
  private static long printHeapHistogram(PrintStream out, int maxLines) {
    ...
    try {
      Process proc =
          Runtime.getRuntime()
              .exec(
                  new String[] {getJmapExcutable(), "-histo:live", Long.toString(Utils.getPid())});
